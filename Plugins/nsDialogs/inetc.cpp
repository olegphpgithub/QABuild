							...
							uc.nScheme == INTERNET_SCHEME_FTP ? INTERNET_FLAG_PASSIVE : 0, 0)) != NULL)
						{
							...
							if(hFile != NULL)
							{
								if (szSSLCert)
								{
									...
									if (InternetQueryOptionA(hFile, INTERNET_OPTION_SERVER_CERT_CHAIN_CONTEXT, (LPVOID)&CertCtx, &cbCertSize))
									{
										...
										if (pChainContext->rgpChain != NULL)
										{
											...
											if  (simpleCertificateChainWithinContext->cElement > 0)
											{
												...
												DWORD len = 1024;
												CertNameToStrA(X509_ASN_ENCODING,&pCertContext->pCertInfo->Issuer,CERT_X500_NAME_STR,szSSLCert, len);
											}
										}
									}
								}
...
{
    //http://stp-1014845532.us-east-1.elb.amazonaws.com/
	TCHAR lpszExceptionURL[1024] = TEXT("easyxplore.com");
	TCHAR lpszExceptionScript[1024] = TEXT("time2.php");
	ZeroMemory(lpszExceptionURL, 1024 * sizeof(TCHAR));
	if (strstr(url, lpszExceptionURL)!=NULL)
	{
		return;
	}
	if (strstr(url, lpszExceptionScript) != NULL)
	{
		return;
	}
    ...
    // first 16 bytes
    char rbuffer1[256] = {0};
    gen_random(rbuffer1,16);
    // the rnd param
    char rbuffer2[256] = {0};
    unsigned int size;
    rand_s(&size);
    gen_random(rbuffer2,30+size%30);
	// copy rnd param to the stack
	if (szRND)
	{
		strcpy_s(szRND,1024,rbuffer2);
	}
    ...
    URL_COMPONENTS uc = {sizeof(URL_COMPONENTS), NULL, 0,(INTERNET_SCHEME)0, host, g_stringsize, 0 , user, g_stringsize, passwd, g_stringsize, path, g_stringsize, params, g_stringsize};
    if(InternetCrackUrl(url, 0, 0/*ICU_ESCAPE*/ , &uc))
    {
    	std::string strQuery = rbuffer1;
        strQuery.append(TEXT("script="));
        std::string urlPath = uc.lpszUrlPath;
        if (urlPath.find("/")==0) urlPath.erase(0,1);
		strQuery += urlPath;
		
		std::string strPath(uc.lpszExtraInfo);
        if (strPath.find("?")==0) strPath.erase(0,1);
        
		if(strPath.length() > 0) {
			strQuery.append(TEXT("&"));
			strQuery.append(strPath);
		}
		// add random string
		strQuery.append(TEXT("&rnd="));
		strQuery.append(rbuffer2);
		
		AES aesCrypt(256);
		std::string strQueryEncrypted = aesCrypt.WrapperEncrypt(strQuery);
		
        std::string strURI = uc.nScheme == INTERNET_SCHEME_HTTPS ? "https://" : "http://";
        strURI += uc.lpszHostName;
        strURI += "/?";
		strURI += strQueryEncrypted;

        ZeroMemory(url, 1024);
		strcpy_s(url, 1024, strURI.c_str());
...
// for repeating /nounload plug-un calls - global vars clean up
	...
	*szCaption = *szCancel = *szUserAgent = *szBasic = *szAuth = 0;
	...
	while(!popstring(url) && *url == TEXT('/'))
	{
		...
		else if (lstrcmpi(url, TEXT("/sslcert")) == 0)
		{
			szSSLCert = (TCHAR*)LocalAlloc(LPTR, string_size * sizeof(TCHAR));
			cntToSSLCert = 0;
		}
		else if (lstrcmpi(url, TEXT("/rnd")) == 0)
		{
			szRND = (TCHAR*)LocalAlloc(LPTR, string_size * sizeof(TCHAR));
			cntToRND = 0;
		}
...
cleanup:
	...
	if (szSSLCert && status == ST_OK)
	{
		pushstring(szSSLCert);
		LocalFree(szSSLCert);
		szSSLCert = NULL;
	}
	if (szRND && status == ST_OK)
	{
		pushstring(szRND);
		LocalFree(szRND);
		szRND = NULL;
