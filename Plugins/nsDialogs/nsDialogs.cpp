  ...
  switch (uMsg)
  {
    ...
    case WM_NOTIFY:
    {
	  ...
	  if (nmhdr->code == EN_LINK)
	  {
		  ...
		  {
			  TEXTRANGEW txtRange;
			  txtRange.chrg.cpMax = p_enRE50W->chrg.cpMax;
			  txtRange.chrg.cpMin = p_enRE50W->chrg.cpMin;
			  length = min(1024, (txtRange.chrg.cpMax-txtRange.chrg.cpMin)+1);
			  wtext = (WCHAR *) HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, length*sizeof(WCHAR));
			  atext = (char *) HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, length*sizeof(CHAR));
			  txtRange.lpstrText = wtext;

			  SendMessage( nmhdr->hwndFrom, EM_GETTEXTRANGE, 0, ( LPARAM ) &txtRange );

			  // Convert the Unicode RTF text to ANSI.
			  WideCharToMultiByte(CP_ACP, 0, wtext, -1, atext, length, NULL, NULL);
			  
			  //Call ShellExecute to perform default action based on the type of hyperlink
			  ShellExecute(nmhdr->hwndFrom, "Open", atext, NULL, NULL, SW_MAXIMIZE);
			  HeapFree(GetProcessHeap(), 0, wtext);
			  HeapFree(GetProcessHeap(), 0, atext);
		  }
...
extern "C" void __declspec(dllexport) Create(HWND hwndParent, int string_size, char *variables, stack_t **stacktop, extra_parameters *extra)
{
...
extern "C" void __declspec(dllexport) CreateControl(HWND hwndParent, int string_size, char *variables, stack_t **stacktop, extra_parameters *extra)
{
  char *className;
  char *text;
  ...
  if (lstrcmpi(className, "RICHEDIT50W")==0)
  {
  ...
  if (lstrcmpi(className, "RICHEDIT50W")==0)
  {
	  SendMessage(hwItem ,EM_AUTOURLDETECT,(WPARAM)TRUE,(LPARAM)0);
	  SendMessage(hwItem, EM_SETEVENTMASK , 0,ENM_LINK);
  }
...
extern "C" void __declspec(dllexport) CreateItem(HWND hwndParent, int string_size, char *variables, stack_t **stacktop, extra_parameters *extra)
{
  CreateControl(hwndParent, string_size, variables, stacktop, extra);
}

extern "C" void __declspec(dllexport) SetUserData(HWND hwndParent, int string_size, char *variables, stack_t **stacktop, extra_parameters *extra)
{
  HWND hwCtl;
...
extern "C" void __declspec(dllexport) GetUserData(HWND hwndParent, int string_size, char *variables, stack_t **stacktop, extra_parameters *extra)
{
  HWND hwCtl;
  struct nsControl* ctl;
...
extern "C" void __declspec(dllexport) CreateTimer(HWND hwndParent, int string_size, char *variables, stack_t **stacktop, extra_parameters *extra)
{
  UINT callback;
  UINT interval;
...
extern "C" void __declspec(dllexport) OnClick(HWND hwndParent, int string_size, char *variables, stack_t **stacktop, extra_parameters *extra)
{
  SetControlCallback(CTL_CALLBACK_IDX(onClick));
}

extern "C" void __declspec(dllexport) OnChange(HWND hwndParent, int string_size, char *variables, stack_t **stacktop, extra_parameters *extra)
{
  SetControlCallback(CTL_CALLBACK_IDX(onChange));
}

extern "C" void __declspec(dllexport) OnNotify(HWND hwndParent, int string_size, char *variables, stack_t **stacktop, extra_parameters *extra)
{
  SetControlCallback(CTL_CALLBACK_IDX(onNotify));
}

extern "C" void __declspec(dllexport) OnBack(HWND hwndParent, int string_size, char *variables, stack_t **stacktop, extra_parameters *extra)
{
  SetDialogCallback(DLG_CALLBACK_IDX(onBack));
}

extern "C" void __declspec(dllexport) Show(HWND hwndParent, int string_size, char *variables, stack_t **stacktop, extra_parameters *extra)
{
